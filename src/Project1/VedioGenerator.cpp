...
void VedioGenerator::Draw_Template(Mat img) {
	//左上定位点
...
void VedioGenerator::Draw_White(Mat img, int pt00, int pt01, int pt10, int pt11)
{
	rectangle(img,
		Point(pt01 * w, pt00 * w),
		Point(pt11 * w, pt10 * w),
...
void VedioGenerator::Draw_Black(Mat img, int pt00, int pt01, int pt10, int pt11)
{
	rectangle(img,
		Point(pt01 * w, pt00 * w),
		Point(pt11 * w, pt10 * w),
...
void VedioGenerator::Draw_White_block(Mat img, int pt0, int pt1) {
	Draw_White(img, pt0, pt1, pt0 + 1, pt1 + 1);
}
void VedioGenerator::Draw_Black_block(Mat img, int pt0, int pt1) {
	Draw_Black(img, pt0, pt1, pt0 + 1, pt1 + 1);
}
int VedioGenerator::Read() {
	int fill[2] = { 237,17 };
	FILE* rfp = NULL;
	errno_t rerr = 0;
...
int VedioGenerator::chksum(void* dataptr, unsigned short len)
{
	unsigned long acc;
	unsigned short src;
